...
#modificando de prev_m para updated_m
        #print(updated_m)
...
class MemoryUpdater(nn.Module):
    ...
    def forward(self, prev_m, input_states, attention_mask):
        ...
        s_t = s_t + prev_m
        #s_t = torch.cat([s_t, prev_m], dim = -1)

        s_t = self.sig(self.saoa1(s_t)) * self.saoa2(s_t)
        ...
        #z_t = torch.sigmoid(self.mz(prev_m) + self.sz(s_t))  # (N, M, D) for not apply SE on Z_t
